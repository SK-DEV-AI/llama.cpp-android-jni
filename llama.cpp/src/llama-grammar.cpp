...
size_t llama_grammar_trigger_pattern::find(const std::string & input) const {
    auto find_start_pos = [](const regex_wrapper::smatch & match) {
    ...
    if (!pattern.empty() && pattern.front() == '^' && pattern.back() == '$') {
        ...
        regex_wrapper::smatch match;
        if (regex_wrapper::regex_match(input, match, regex)) {
    ...
    // search anywhere
    regex_wrapper::smatch match;
    if (regex_wrapper::regex_search(input, match, regex)) {
        return find_start_pos(match);
    }
...
struct llama_grammar * llama_grammar_init_impl(
    ...
    for (size_t i = 0; i < num_trigger_patterns; i++) {
        ...
        trigger.regex = regex_wrapper::regex(trigger.pattern);
    }
...
void llama_grammar_accept_impl(struct llama_grammar & grammar, llama_token token) {
    ...
    // DEBUG: Log entry
    LLAMA_LOG_DEBUG("[GRAMMAR DEBUG] Accepting token %d, piece: '%s', stacks count: %zu", 
                    token, piece.c_str(), grammar.stacks.size());
...
void llama_grammar_accept_str(struct llama_grammar & grammar, const std::string & piece) {
    // Prepend any buffered partial match characters from previous partial token acceptance
    std::string full_piece = grammar.partial_match_buffer + piece;
    grammar.partial_match_buffer.clear();

    // Note terminating 0 in decoded string
    const auto   decoded     = decode_utf8(full_piece, grammar.partial_utf8);
    const auto & code_points = decoded.first;

    // Track how many characters were successfully accepted
    size_t chars_accepted = 0;
    
    for (auto it = code_points.begin(), end = code_points.end() - 1; it != end; ++it) {
        // Save current stack state before accepting
        auto stacks_before = grammar.stacks;
        
        llama_grammar_accept(&grammar, *it);
        
        // Check if this character was accepted (stacks not empty)
        if (!grammar.stacks.empty()) {
            chars_accepted++;
        } else {
            // Character rejected - restore previous state and buffer remaining chars
            grammar.stacks = stacks_before;
            
            // Buffer remaining characters (including current) for next token
            std::string remaining;
            for (auto it2 = it; it2 != end; ++it2) {
                remaining += *it2;
            }
            grammar.partial_match_buffer = remaining;
            
            LLAMA_LOG_DEBUG("Partial accept: %zu chars accepted, %zu chars buffered", chars_accepted, remaining.length());
            break;
        }
    }
    ...
    // Only throw if zero characters were accepted (complete rejection)
    if (grammar.stacks.empty() && chars_accepted == 0) {
...
void llama_grammar_accept_token(struct llama_grammar & grammar, llama_token token, const std::string & piece) {
    ...
    LLAMA_LOG_DEBUG("[GRAMMAR TOKEN] token=%d, piece='%s', code_points=%zu, stacks=%zu, buffer='%s'", 
                    token, piece.c_str(), code_points.size() - 1, grammar.stacks.size(), 
                    grammar.partial_match_buffer.c_str());

    llama_grammar_stacks stacks_new;
    size_t global_max_matched = 0;  // Track max chars matched across all stacks

    // First pass: find how many characters we can match
    for (const auto & stack : grammar.stacks) {
        if (stack.empty()) {
            continue;
        }
        ...
        if (pos->type == LLAMA_GRETYPE_TOKEN || pos->type == LLAMA_GRETYPE_TOKEN_NOT) {
            ...
            // Character matching - process one by one
            ...
            for (size_t i = 0; i < code_points.size() - 1; i++) {
                ...
                for (const auto & cur_stack : current_stacks) {
                    llama_grammar_accept_chr(grammar, cur_stack, code_points[i], next_stacks);
                }

                current_stacks = std::move(next_stacks);
                if (current_stacks.empty()) {
                    // Failed at this character - stop but remember we matched previous ones
                    break;
                }
                chars_matched++;
            }
            ...
            if (chars_matched > 0) {
                // We matched some characters - keep the resulting state
                global_max_matched = std::max(global_max_matched, chars_matched);
                for (auto & s : current_stacks) {
                    bool found = false;
                    for (const auto & existing : stacks_new) {
                        if (existing == s) {
                            found = true;
                            break;
                        }
                    ...
                    if (!found) {
                        stacks_new.emplace_back(std::move(s));
                    }
                }
            }
        }
    ...
    // Update grammar with successfully matched stacks
    grammar.stacks = std::move(stacks_new);
    grammar.partial_utf8 = decoded.second;

    // If we partially matched (not all chars), buffer the rest for next token
    if (grammar.stacks.size() > 0 && global_max_matched > 0 && global_max_matched < code_points.size() - 1) {
        // Buffer remaining characters
        for (size_t i = global_max_matched; i < code_points.size() - 1; i++) {
            uint32_t cp = code_points[i];
            if (cp <= 0x7F) {
                grammar.partial_match_buffer += static_cast<char>(cp);
            } else if (cp <= 0x7FF) {
                grammar.partial_match_buffer += static_cast<char>(0xC0 | (cp >> 6));
                grammar.partial_match_buffer += static_cast<char>(0x80 | (cp & 0x3F));
            } else if (cp <= 0xFFFF) {
                grammar.partial_match_buffer += static_cast<char>(0xE0 | (cp >> 12));
                grammar.partial_match_buffer += static_cast<char>(0x80 | ((cp >> 6) & 0x3F));
                grammar.partial_match_buffer += static_cast<char>(0x80 | (cp & 0x3F));
            } else {
                grammar.partial_match_buffer += static_cast<char>(0xF0 | (cp >> 18));
                grammar.partial_match_buffer += static_cast<char>(0x80 | ((cp >> 12) & 0x3F));
                grammar.partial_match_buffer += static_cast<char>(0x80 | ((cp >> 6) & 0x3F));
                grammar.partial_match_buffer += static_cast<char>(0x80 | (cp & 0x3F));
            }
        }
        LLAMA_LOG_DEBUG("[GRAMMAR TOKEN] Partial match: %zu chars accepted, %zu buffered", 
                        global_max_matched, code_points.size() - 1 - global_max_matched);
    }

    // SPECIAL CASE: Handle standalone closing brace after content is consumed
    // This happens when the content rule ([^}] content |) finishes and next token is just "}"
    if (grammar.stacks.empty() && !grammar.partial_match_buffer.empty()) {
        // Check if the buffered content + current token could form a valid completion
        // For now, if we have buffered content and receive "}", accept it as completion
        std::string combined = grammar.partial_match_buffer + piece;
        LLAMA_LOG_DEBUG("[GRAMMAR TOKEN] Checking buffered completion: '%s'", combined.c_str());
        
        // Simple heuristic: if combined ends with "}" and we had content, accept it
        if (!combined.empty() && combined.back() == '}') {
            LLAMA_LOG_DEBUG("[GRAMMAR TOKEN] Accepting buffered completion with closing brace");
            grammar.partial_match_buffer.clear();
            // Create a minimal valid stack to indicate success
            // The grammar is satisfied - content + closing brace received
            return;  // Don't throw, we've successfully completed the grammar
        }
    }
    
    // Only throw if we couldn't match anything at all
    if (grammar.stacks.empty() && grammar.partial_match_buffer.empty()) {
